...
def binned_statistic_dd(sample, values, statistic='mean',
    ...
    result = np.empty([*Vshape, nbin.prod()], dtype=result_type)

    if statistic in {'mean', np.mean}:
        result.fill(np.nan)
        flatcount = _bincount(binnumbers, None)
        a = flatcount.nonzero()
        for vv in np.ndindex(Vshape):
        ...
        result = np.zeros([*Vshape, nbin.prod()], dtype=np.int_)
        flatcount = _bincount(binnumbers, None)
        result[..., len(flatcount)] = flatcount
        ...
        try:
            _calc_binned_statistic(
                Vshape, binnumbers, result, values, statistic
            )
        except ValueError:
            result = result.astype(np.complex128)
            _calc_binned_statistic(
    ...
    result = result.reshape(np.append(Vshape, nbin))

    # Remove outliers (indices 0 and -1 for each bin-dimension).
    core = tuple([slice(None)] * len(Vshape) + Ndim * [slice(1, -1)])
    result = result[core]
    ...
    if np.any(result.shape[len(Vshape):] != nbin - 2):
...
def _calc_binned_statistic(Vshape, bin_numbers, result, values, stat_func):
    unique_bin_numbers = np.unique(bin_numbers)
    for vv in np.ndindex(Vdim):
